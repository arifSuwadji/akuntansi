  ...
  'columns' : [
    ...
    { "mRender": function(data, type, full){
      if(full['deposit_tipe']){
        return '<a href="/transaksi/deposit/editTipe/'+full['deposit_id']+'/'+full['jurnal']+'"> <span>'+full['nama_deposit']+'</span></a>';
      }else{
        return '<select class="form-control select2 tipeDeposit" onchange="updateTipe('+full['deposit_id']+')" style="width: 100%;" name="akun_perkiraan" data-placeholder="Pilih Tipe Deposit"><option></option><option value="1">Deposit Hutang</option><option value="2">Deposit BCA 117-048-8888 an/Tedi Hartono</option><option value="3">Deposit BCA 117-027-7771 an/Tedi Hartono</option><option value="4">Deposit MANDIRI 11-4000-999-888-4 a/n TEDI HARTONO</option><option value="5">Deposit BRI 0130-01-000296-56-5 an/ Lia Faroka</option><option value="6">Deposit Kas Utama</option></select>';
      }
    }},
  ]
});
...
$("#cari").on("click", function(event){
  ...
  table.search(tanggal+'_'+nama+'_'+catatan).draw();
});
