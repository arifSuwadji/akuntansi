...
let tablePendapatan = $('#tableDataPendapatan').DataTable({
  ...
  'processing'  : true,
  'serverSide'  : true,
  'ajax' : '/laporan/akuntansi/labarugi/harian/pendapatanJson',
  "order": [[ 1, "asc" ]],
  'pageLength' : 100,
  "dom" : 'tr',
  'columns' : [
    ...
    { "mRender" : function(data, type, full){
      if(full['tipe_akun'] == 'induk'){
        return '<span class="pull-right text-bold" id="saldoMutasi'+full['akun_id']+'">'+saldoMutasiPendapatan(full['akun_id'], full['tipe_akun'], full['kode_akun'])+'</span>';
      }else{
        return '<span class="pull-right" id="saldoMutasi'+full['akun_id']+'">'+saldoMutasiPendapatan(full['akun_id'], full['tipe_akun'], full['kode_akun'])+'</span>';
      }
    }},
    { "mRender" : function(data, type, full){
      if(full['tipe_akun'] == 'induk'){
...
let tableBeban = $('#tableDataBeban').DataTable({
  ...
  'processing'  : true,
  'serverSide'  : true,
  'ajax' : '/laporan/akuntansi/labarugi/harian/bebanJson',
  "order": [[ 1, "asc" ]],
  'pageLength' : 100,
  "dom" : 'tr',
  'columns' : [
    ...
    { "mRender" : function(data, type, full){
      if(full['tipe_akun'] == 'induk'){
        return '<span class="pull-right text-bold" id="saldoMutasi'+full['akun_id']+'">'+saldoMutasiBeban(full['akun_id'], full['tipe_akun'], full['kode_akun'])+'</span>';
      }else{
        return '<span class="pull-right" id="saldoMutasi'+full['akun_id']+'">'+saldoMutasiBeban(full['akun_id'], full['tipe_akun'], full['kode_akun'])+'</span>';
      }
    }},
    { "mRender" : function(data, type, full){
      if(full['tipe_akun'] == 'induk'){
...
let getSaldoAwal = function (idAkun, tipeAkun, kodeAkun, callback){
  let dmy1 = $('#dmy1').val();
  $.get('/laporan/akuntansi/labarugi/harian/saldoAkun/'+idAkun+'/'+tipeAkun+'/'+kodeAkun, {tanggal:dmy1}, function(data, status){
...
(function($, viewport){
    $(document).ready(function() {
        ...
        if(viewport.is('>=sm')) {
            // ...
            let column = tablePendapatan.column(5);
            column.visible(!column.visible());
            column = tableBeban.column(5);
            column.visible(!column.visible());
        }
        ...
        if(viewport.is('<md')) {
            ...
            column.visible(!column.visible());
            column = tablePendapatan.column(3);
            column.visible(!column.visible());
            column = tablePendapatan.column(4);
            column.visible(!column.visible());
            //pasiva
