...
let tableAktiva = $('#tableDataAktiva').DataTable({
  ...
  'processing'  : true,
  'serverSide'  : true,
  'ajax' : '/laporan/akuntansi/neraca/bulanan/aktivaJson',
  "order": [[ 1, "asc" ]],
  'pageLength' : 100,
  "dom" : 'tr',
  'columns' : [
...
let tablePasiva = $('#tableDataPasiva').DataTable({
  ...
  'processing'  : true,
  'serverSide'  : true,
  'ajax' : '/laporan/akuntansi/neraca/bulanan/pasivaJson',
  "order": [[ 1, "asc" ]],
  'pageLength' : 100,
  "dom" : 'tr',
  'columns' : [
    ...
    { "mRender" : function(data, type, full){
      if(full['tipe_akun'] == 'induk'){
        ...
        return '<div class="col-md-3">'+' <div class="box box-primary box-solid">'+' <div class="box-header with-border">'+' <h3 class="box-title">'+full['kode_akun']+'</h3>'+'<div class="box-tools pull-right">'+' </div>'+' </div>'+' <div class="box-body"><h5>'+full['nama_akun']+'</h5><h5>Saldo: <span id="saldoAwal'+full['akun_id']+'"></span></div>'+' </div>'+' </div>';
      }
    }}
  ]
});
...
$("#cari").on("click", function(event){
  ...
  tableAktiva.search(drTgl).draw();
  tablePasiva.search(drTgl).draw();
});

let getSaldoAwal = function (idAkun, tipeAkun, kodeAkun, callback){
  let dmy1 = $('#dmy1').val();
  $.get('/laporan/akuntansi/neraca/bulanan/saldoAkun/'+idAkun+'/'+tipeAkun+'/'+kodeAkun, {tanggal:dmy1}, function(data, status){
...
(function($, viewport){
    $(document).ready(function() {
        ...
        if(viewport.is('>=sm')) {
            // ...
            let column = tableAktiva.column(3);
